...
def apply_direction_constraint(points, movable, p1_indices, p2_indices):
    """应用特定方向的约束
    ...
    # 获取各点的可移动状态
    m1_all = movable[i1, j1]
    m2_all = movable[i2, j2]
    ...
    # 应用调整量到坐标
    new_points[i1, j1, 0] += delta_p1_x_both + delta_p1_x_only
    new_points[i1, j1, 1] += delta_p1_y_both + delta_p1_y_only
    new_points[i2, j2, 0] += delta_p2_x_both + delta_p2_x_only
    new_points[i2, j2, 1] += delta_p2_y_both + delta_p2_y_only
...
def apply_constraints(points, movable, mouse_pos=None, mouser=1e-5):
    ...
    for _ in range(constraint_iterations):
        ...
        i2 = i1 + 1
        j2 = j1
        new_points = apply_direction_constraint(new_points, movable, (i1, j1), (i2, j2))

        apply_mouse(mouse_pos, mouser)
        # 2. 对角线方向约束
        for layer_idx in range(points.shape[0] - 1):
            ...
            if len(valid_j1) > 0:
                ...
                new_points = apply_direction_constraint(new_points, movable, (i1, valid_j1), (i2, valid_j2))
