...
def apply_constraints(points, movable):
    ...
    for _ in range(constraint_iterations):
        # 垂直约束（层间约束）
        # 提取所有相邻层对
        p1_all = new_points[:-1]  # (19, 20, 4)
        p2_all = new_points[1:]  # (19, 20, 4)

        # 计算坐标差
        dx = p2_all[..., 0] - p1_all[..., 0]  # (19, 20)
        dy = p2_all[..., 1] - p1_all[..., 1]

        # 计算弹簧力误差及方向调整量
        distance = np.hypot(dx, dy)
        err = (distance - spring_rest_length) * spring_stiffness/distance
        delta_x = dx * err
        delta_y = dy * err

        # 获取各层点的可移动状态
        m1_all = movable[:-1]  # (19, 20)
        m2_all = movable[1:]  # (19, 20)

        # 计算三种移动条件
        both_movable = m1_all & m2_all
        only_p1 = m1_all & ~m2_all
        only_p2 = ~m1_all & m2_all

        # 计算各情况对坐标的调整量
        # 双方可移动的调整量
        delta_p1_x_both = 0.5 * delta_x * both_movable
        delta_p1_y_both = 0.5 * delta_y * both_movable
        delta_p2_x_both = -0.5 * delta_x * both_movable
        delta_p2_y_both = -0.5 * delta_y * both_movable

        # 仅p1可移动的调整量
        delta_p1_x_only = delta_x * only_p1
        delta_p1_y_only = delta_y * only_p1

        # 仅p2可移动的调整量
        delta_p2_x_only = -delta_x * only_p2
        delta_p2_y_only = -delta_y * only_p2

        # 初始化总调整量矩阵
        delta_x_total = np.zeros_like(new_points[:, :, 0])  # (20, 20)
        delta_y_total = np.zeros_like(new_points[:, :, 1])

        # 累加p1层的调整量（处理所有层对的左端点）
        delta_x_total[:-1] += delta_p1_x_both + delta_p1_x_only
        delta_y_total[:-1] += delta_p1_y_both + delta_p1_y_only

        # 累加p2层的调整量（处理所有层对的右端点）
        delta_x_total[1:] += delta_p2_x_both + delta_p2_x_only
        delta_y_total[1:] += delta_p2_y_both + delta_p2_y_only

        # 应用总调整量到坐标
        new_points[:, :, 0] += delta_x_total
        new_points[:, :, 1] += delta_y_total
