...
def optimize_spring_constraints(points, pointsmove, constraint_iterations, spring_rest_length, spring_stiffness):
    """
    将输入数据转换为numpy数组并运行优化后的约束处理
    :param points: 多层点阵结构 (层数, 点数, 4) 的numpy数组，每个点包含[x, y, _0, _1]
    :param pointsmove: 点移动性标记 (层数, 点数) 的布尔数组
    :return: 更新后的points数组
    """
    # 转换为可写视图以确保numba可以修改数组
    points = np.ascontiguousarray(points)

    @njit
    def apply_constraints(points, pointsmove, constraint_iterations, rest_length, stiffness):
        num_layers = points.shape[0]
        for _ in range(constraint_iterations):
            # 水平方向约束
            for layer_idx in range(num_layers - 1):
                current_layer = points[layer_idx]
                next_layer = points[layer_idx + 1]
                num_points = current_layer.shape[0]

                for point_idx in range(num_points):
                    if point_idx >= next_layer.shape[0]:
                        continue  # 处理不同点数的情况

                    # 解包数据
                    p1x = current_layer[point_idx, 0]
                    p1y = current_layer[point_idx, 1]
                    p2x = next_layer[point_idx, 0]
                    p2y = next_layer[point_idx, 1]

                    # 获取移动标记
                    pi_move = pointsmove[layer_idx, point_idx]
                    p2_move = pointsmove[layer_idx + 1, point_idx]
                    ...
                    displacement = (distance - rest_length) * stiffness / distance
                    delta_x = dx * displacement
                    delta_y = dy * displacement

                    # 更新坐标
                    if pi_move and p2_move:
                        current_layer[point_idx, 0] += delta_x / 2
                        current_layer[point_idx, 1] += delta_y / 2
                        next_layer[point_idx, 0] -= delta_x / 2
                        next_layer[point_idx, 1] -= delta_y / 2
                    elif pi_move:
                        current_layer[point_idx, 0] += delta_x
                        current_layer[point_idx, 1] += delta_y
                    elif p2_move:
                        next_layer[point_idx, 0] -= delta_x
                        next_layer[point_idx, 1] -= delta_y

            # 垂直方向约束
            for layer_idx in range(num_layers - 1):
                current_layer = points[layer_idx]
                next_layer = points[layer_idx + 1]
                direction = -1 if (layer_idx + 1) % 2 else 1
                start_idx = (layer_idx + 1) % 2
                end_idx = current_layer.shape[0] - (layer_idx % 2)

                for point_idx in range(start_idx, end_idx):
                    paired_idx = point_idx + direction
                    if paired_idx < 0 or paired_idx >= next_layer.shape[0]:
                        continue

                    # 解包数据
                    p1x = current_layer[point_idx, 0]
                    p1y = current_layer[point_idx, 1]
                    p2x = next_layer[paired_idx, 0]
                    p2y = next_layer[paired_idx, 1]

                    # 获取移动标记
                    pi_move = pointsmove[layer_idx, point_idx]
                    p2_move = pointsmove[layer_idx + 1, paired_idx]
                    ...
                    if pi_move and p2_move:
                        current_layer[point_idx, 0] += delta_x / 2
                        current_layer[point_idx, 1] += delta_y / 2
                        next_layer[paired_idx, 0] -= delta_x / 2
                        next_layer[paired_idx, 1] -= delta_y / 2
                    elif pi_move:
                        current_layer[point_idx, 0] += delta_x
                        current_layer[point_idx, 1] += delta_y
                    elif p2_move:
                        next_layer[paired_idx, 0] -= delta_x
                        next_layer[paired_idx, 1] -= delta_y

        return points

    return apply_constraints(points, pointsmove, constraint_iterations, spring_rest_length, spring_stiffness)
...
for i in range(20):
    points.append([])
    pointsmove.append([True] * 20)
    for j in range(20):
        points[i].append([xp + rk * j + rk * (i % 2) / 2, yp - rk * i * 0.75 ** 0.5,
                          xp + rk * j + rk * (i % 2) / 2, yp - rk * i * 0.75 ** 0.5])

# 标记固定点
for i in range(len(pointsmove)):
    pointsmove[0][i] = False

points_array = np.array([[p[0], p[1], p[2], p[3]] for layer in points for p in layer], dtype=np.float64)
pointsmove_array = np.array([[pm for pm in layer] for layer in pointsmove], dtype=np.bool_)
...
while True:
    ...
    points = optimize_spring_constraints(points_array, pointsmove_array, constraint_iterations, spring_rest_length, spring_stiffness)
