...
# points_array = np.array([[[p[0], p[1], p[2], p[3]] for layer in original_points for p in layer], dtype=np.float64)
# pointsmove_array = np.array([[pm for pm in layer] for layer in original_pointsmove], dtype=np.bool_)

# 调用优化函数
# optimized_points = optimize_spring_constraints(points_array, pointsmove_array, constraint_iterations, spring_rest_length, spring_stiffness)
